...
// Build evening reminder SMS (varies by streak for loss aversion)
export function buildReminderSMS({ runner, streak, dayNumber }) {
  const firstName = runner.name.split(' ')[0];
  const link = dayNumber
    ? `${BASE_URL}/day/${runner.coaching_token}?d=${dayNumber}`
    : `${BASE_URL}/dashboard/${runner.coaching_token}`;

  if (streak === 0) {
    return `Hey ${firstName}, today's action takes 2 min. Start building your streak!\n${link}`;
  }
  if (streak <= 2) {
    return `Hey ${firstName}, you're building something. Today takes 2 min. Don't stop now.\n${link}`;
  }
  if (streak <= 6) {
    return `${firstName}, ${streak}-day streak! Don't let it end today. 2 min is all it takes.\n${link}`;
  }
  if (streak <= 13) {
    return `${firstName}, ${streak} days straight. Most people never get here. Keep it alive.\n${link}`;
  }
  return `${firstName}, ${streak}-day streak. That's rare. Don't stop now.\n${link}`;
}
